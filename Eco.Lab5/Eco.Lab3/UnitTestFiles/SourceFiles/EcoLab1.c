...
int16_t EcoMain(IEcoUnknown* pIUnk) {
    ...
    IFirstInFirstOut* pIFIFO = 0;
    uint32_t pages[] = {1, 2, 3, 4, 2, 1, 5, 6, 2, 1, 2, 3, 7, 6, 3, 2, 1, 2, 3, 6, 7};
    ...
    result = pIBus->pVTbl->QueryComponent(pIBus, &CID_FirstInFirstOut, 0, &IID_IFirstInFirstOut, (void**) &pIFIFO);
    if (result != 0 || pIFIFO == 0) {
        goto Release;
    }
    ...
    pIFIFO->pVTbl->Init(pIFIFO, 4);

    pIFIFO->pVTbl->Print_Header(pIFIFO);
    pIFIFO->pVTbl->Print_Report_Line(pIFIFO, 0);
    for (i = 0; i < sizeof(pages) / sizeof(pages[0]); ++i) {
        pIFIFO->pVTbl->Access_Page(pIFIFO, pages[i]);
        pIFIFO->pVTbl->Print_Report_Line(pIFIFO, pages[i]);
    }

    printf("\n");
    pIFIFO->pVTbl->Print_Final_Line(pIFIFO);
    printf("\n\n");
