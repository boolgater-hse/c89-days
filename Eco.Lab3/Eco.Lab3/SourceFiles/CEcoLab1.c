 ...
 *
 * <сводка>
 *   Функция Fire_OnTreePrintCallback
 * </сводка>
 *
 * <описание>
 ...
 *
 */
int16_t ECOCALLMETHOD CEcoLab1_Fire_OnTreePrintCallback(/* in */ struct IEcoLab1* me, /* in */ struct RBNode* p_Root, /* in */ int16_t bClear) {
    CEcoLab1* pCMe = (CEcoLab1*) me;
    int16_t result = 0;
 ...
 *
 */
int16_t ECOCALLMETHOD CEcoLab1_Fire_OnTreePrintWithHighlight(/* in */ struct IEcoLab1* me,
 ...
 *
 * <сводка>
 *   Функция Fire_OnSaveCursorPositionCallback
 * </сводка>
 *
 * <описание>
 *   Функция вызова обратного интерфейса
 ...
 *
 */
int16_t ECOCALLMETHOD CEcoLab1_Fire_OnSaveCursorPositionCallback(/* in */ struct IEcoLab1* me) {
    CEcoLab1* pCMe = (CEcoLab1*) me;
    int16_t result = 0;
    IEcoEnumConnections* pEnum = 0;
    IEcoLab1Events* pIEvents = 0;
    ...
    if (pCMe->m_pISinkCP != 0) {
        ...
        if ((result == 0) && (pEnum != 0)) {
            while (pEnum->pVTbl->Next(pEnum, 1, &cd, 0) == 0) {
                result = cd.pUnk->pVTbl->QueryInterface(cd.pUnk, &IID_IEcoLab1Events, (void**) &pIEvents);
                if ((result == 0) && (pIEvents != 0)) {
                    result = pIEvents->pVTbl->OnSaveCursorPositionCallback(pIEvents);
                    pIEvents->pVTbl->Release(pIEvents);
                }
                cd.pUnk->pVTbl->Release(cd.pUnk);
            }
 ...
 *
 * <сводка>
 *   Функция Fire_OnTreeInorderTraverseCallback
 * </сводка>
 *
 * <описание>
 *   Функция вызова обратного интерфейса
 ...
 *
 */
int16_t ECOCALLMETHOD CEcoLab1_Fire_OnTreeInorderTraverseCallback(/* in */ struct IEcoLab1* me, /* in */ struct RBNode* p_Root) {
    CEcoLab1* pCMe = (CEcoLab1*) me;
    int16_t result = 0;
    IEcoEnumConnections* pEnum = 0;
    IEcoLab1Events* pIEvents = 0;
    ...
    if (pCMe->m_pISinkCP != 0) {
        ...
        if ((result == 0) && (pEnum != 0)) {
            while (pEnum->pVTbl->Next(pEnum, 1, &cd, 0) == 0) {
                result = cd.pUnk->pVTbl->QueryInterface(cd.pUnk, &IID_IEcoLab1Events, (void**) &pIEvents);
                if ((result == 0) && (pIEvents != 0)) {
                    result = pIEvents->pVTbl->OnTreeInorderTraversalCallback(pIEvents, p_Root);
                    pIEvents->pVTbl->Release(pIEvents);
                }
                cd.pUnk->pVTbl->Release(cd.pUnk);
            }
 ...
 *
 * <описание>
 *   Performs Tree sort on array p_Base of size u32Num with one element byte size
 *   u32Size. Elements are comparing by fnCompare comparator.
 * </описание>
 *
 */
int16_t ECOCALLMETHOD CEcoLab1_TreeSort(/* in */ struct IEcoLab1* me,
                                        /* in */ void* p_Base,
                                        /* in */ size_t u32Num,
                                        /* in */ size_t u32Size,
                                        /* in */ int (ECOCDECLMETHOD* fnCompare)(const void*, const void*)) {
    CEcoLab1* pCMe = (CEcoLab1*) me;
    struct RBNode* p_RBT = 0;
    size_t u32Index = 0;
    int i = 0;

    CEcoLab1_Fire_OnSaveCursorPositionCallback(me);

    /* Проверка указателей */
    if (me == 0 || p_Base == 0 || fnCompare == 0 || u32Num <= 0 || u32Size <= 0) {
        return -1;
    }

    for (i = 0; i < u32Num; ++i) {
        p_RBT = Insert(me, &p_RBT, (char_t*) p_Base + (u32Size * i), u32Size, fnCompare);
    }

    CEcoLab1_Fire_OnTreePrintCallback(me, p_RBT, 0);

    Write_Inorder(me, p_RBT, p_Base, u32Size, &u32Index);
    CEcoLab1_Fire_OnTreeInorderTraverseCallback(me, p_RBT);

    CEcoLab1_Fire_OnTreePrintCallback(me, p_RBT, 2);
